...
const users = JSON.parse(fs.readFileSync(path.join(__dirname, 'endusers.json'), 'utf8'));

// Load admin credentials from a file
const admins = JSON.parse(fs.readFileSync(path.join(__dirname, 'admin.json'), 'utf8'));
...
app.post('/start-server', (req, res) => {
    if (!serverProcess) {
        serverProcess = exec(`node ${path.join(__dirname, 'server.js')}`, (error, stdout, stderr) => {
            if (error) {
